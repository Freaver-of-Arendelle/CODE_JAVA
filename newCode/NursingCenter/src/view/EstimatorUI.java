...
public class EstimatorUI extends JFrame {
    ...
    public EstimatorUI(Vector<Patient> patients, String username) {
        EstimatorUI.username = username;
        initComponents();
        selInit(selCB, vector);
        refresh(((DefaultTableModel) table1.getModel()), patients);
    }
    ...
    public static void init(DefaultTableModel model, Vector<Patient> vector) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            vector1.add(null);
            vector1.add(null);
            vector1.add(new Date());
            vector1.add(username);
            vector1.add(null);
    ...
    private void startBtnActionPerformed(ActionEvent e) {
        ...
        for (Template template : vector) {
            if (template.getTitle().equals(title)) {
                EvaluationUI ui = new EvaluationUI(template);
                ui.setVisible(true);
                return;
            }
        }
