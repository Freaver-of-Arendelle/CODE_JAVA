...
public class TemplateListUI extends JFrame {
    ...
    private static  Vector<Template> vector = new Vector<>(controller.getTemplates());
...
//        model = new DefaultComboBoxModel(new Vector(strings));
    ...
    private static void init1(DefaultTableModel model, Vector<Template> vector) {
        Set<Template> objects = new TreeSet<>(vector);
...
//        template.setProblems(new HashSet<>(tempVector));
    ...
    private void delBTnActionPerformed(ActionEvent e) {
        DefaultTableModel model = (DefaultTableModel) table1.getModel();
        Vector vector1 = model.getDataVector().get(table1.getSelectedRow());

        TemplateListUI.vector.removeIf(c -> c.getId().equals(vector1.get(1)));
        model.removeRow(table1.getSelectedRow());
//        for (int i = 0; i < table1.getRowCount(); i++) {
//            if (table1.getValueAt(i, 0).equals(true)) {
//                int id = (int) table1.getValueAt(i, 1);
//                Template template = vector.get(i);
//                if (template.getProblems() != null) {
...
//            }
//        }
        selRefresh(((DefaultComboBoxModel) selCB.getModel()), TemplateListUI.vector);
        refresh1(model, TemplateListUI.vector);
