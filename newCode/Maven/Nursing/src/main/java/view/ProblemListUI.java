    ...
    private void newBtnActionPerformed(ActionEvent e) {
        ...
        for (Template template1 : templates) {
            ...
            for (Vector vector : dataVector) {
                problems.add(new Problem(((Integer) vector.get(1)), ((String) vector.get(2)),
                        null, ((String) vector.get(3))));
            }
        }
        TemplateController singleton = TemplateController.getSingleton();
...
//        table1.editCellAt(count - 1, 0);
    ...
    private void okBtnActionPerformed(ActionEvent e) {
        ...
        Set<Problem> tempProblems = new LinkedHashSet<>(vector);
        controller.clear();
        for (Vector vector1 : dataVector) {
            Problem problem = new Problem(((Integer) vector1.get(1)), ((String) vector1.get(2)), null,
                    ((String) vector1.get(3)));
            tempProblems.add(problem);
        }


        for (Problem tempProblem : tempProblems) {
            controller.addProblem(tempProblem);
        }

//        Set<Template> templates = singleton.getTemplates();
//        List<Template> list = new ArrayList<>(templates);
//        for (int i = 0; i < list.size(); i++) {
//            if (list.get(i).getId() == tempId) {
//                Set<Problem> problems = list.get(i).getProblems();
