...
public class GourmetCoffee  {
	...
	public void displayNumberOfOrders(Product product) {
		int num = 0;
		Iterator<Order> ito = this.sales.iterator();
		Iterator<OrderItem> itoi;
		while (ito.hasNext()) {
			Order order = (Order) ito.next();
			itoi = order.iterator();
			while (itoi.hasNext()) {
				OrderItem orderItem = (OrderItem) itoi.next();
				if (orderItem.getProduct().equals(product)) {
					num++;
				}
			}
		}
		System.out.println("Number of orders that contains the product"+product.getCode()+":"+num);
	...
	public void displayTotalQuantityOfProducts() {
		...
		Iterator<Order> ito = this.sales.iterator();
		Iterator<OrderItem> itoi;
		Iterator<Product> itop = this.catalog.iterator();
		while (itop.hasNext()) {
			ito = this.sales.iterator();
			Product product2 = (Product) itop.next();
			while (ito.hasNext()) {
				Order order = (Order) ito.next();
				itoi = order.iterator();
				while (itoi.hasNext()) {
					OrderItem orderItem = (OrderItem) itoi.next();
					if (orderItem.getProduct().equals(product2)) {
						num += orderItem.getQuantity();
					}
				}
			}
			System.out.println(product2.getCode()+" "+num);
			num = 0;
		}
